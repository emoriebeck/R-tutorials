...
soep_long <- soep %>%
  ...
  mutate(value = as.numeric(value), # change to numeric
         value = ifelse(value  < 0, NA, value), # recode missings to NA
         value = ifelse(reverse == -1, reverse.code(-1, value, mini = mini, maxi = maxi), value))
```
...
b5_soep_long <- soep_long %>%
  filter(type == "Big 5") %>%
  group_by(Procedural__SID, item, year) %>% 
  summarize(value = mean(value, na.rm = T)) %>%
  left_join(soep_long %>% 
    select(Procedural__SID, DOB = Demographic__DOB.2005, Sex = Demographic__Sex.2005) %>%
    distinct()) %>%
  filter(!is.na(DOB))
```
