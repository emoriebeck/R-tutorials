...
change_fun <- function(file){
  sprintf("%s/%s", data_path, file) %>% 
    ...
    select(-scrap) %>%
    spread(tmp, value) %>%
    write.csv(., file = sprintf("%s/%s", data_path, file))
}
...
IDs <- grep("ALDA/week_3_purrr/data/example_1/", filelist, value = TRUE, fixed = TRUE)[1:10]
```
...
tibble(ID = list.files(sprintf("%s/data/example_1", data_path))) %>%
...
```{r}
old.names <- codebook$old.names
new.names <- codebook$new.names
data_path <- ""
df3 <- tibble(wave = list.files(data_path)) %>%
  mutate(data = map(ID, read_csv),
         wave = as.numeric(str_extract_all(., "[0-9]"))) %>%
  unnest(data) %>%
  select(old.names) %>%
  setNames(new.names)
```
...
read_fun <- function(wave){
  old.names <- (codebook %>% filter(Year == year | Year == 0))$old_names
  new.names <- (codebook %>% filter(Year == year | Year == 0))$new_names
  
  sprintf("%s/data/target_w%s.csv", wd, wave) %>%
    read.csv() %>%
...
## Multiple Waves, Multiple Files for Same Variables  
