...
download.file(
  ...
  destfile = destfile = sprintf("%s/data/codebook.csv", data_path)
  )

# load the codebook
(codebook <- sprintf("%s/data/codebook.csv", wd) %>% 
    read_csv(.) %>%
...
download.file(
  ...
  destfile = destfile = sprintf("%s/data/workflow_data.csv", data_path)
  )

old.names <- codebook$old_name # get old column names
new.names <- codebook$new_name # get new column names

(soep <- sprintf("%s/data/workflow_data.csv", wd) %>% # path to data
...
```{r}
events_long %>%
  mutate(value = mapvalues(value, 0:1, c("No Event", "Event"))) %>%
