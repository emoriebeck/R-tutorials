...
```{r}
library(lme4)
(unconditional_model <- unique(dat_final %>% select(PROC_SID:BF_O, Psych_LifeSat)) %>%
  gather(key = Trait, value = value, BF_A:BF_O, na.rm = T) %>%
...
unconditional_model <- unconditional_model %>% 
  mutate(mod1 = map(data, ~lmer(value ~ Year + Psych_LifeSat + Year*Psych_LifeSat + 
                                  (1|PROC_SID), data = .)),
         summary1 = map(mod1, ~print(summary(.))))
```


Now, let's add a random slope and compare the models

```{r}
# random slope and intercept
unconditional_model <- unconditional_model %>% 
  mutate(mod2 = map(data, ~lmer(value ~ Year + Psych_LifeSat + Year*Psych_LifeSat + 
                                  (Year | PROC_SID), data = .)),
...
nested.mods %>% unnest(pred1) %>%
  ...
  ggplot(aes(x = Year, y = pred, color = group)) +
  geom_line() + 
  facet_wrap(~Trait, nrow = 2) +
  theme_classic()
```
...
dat_final %>% 
  filter(PROC_SID %in% sample_subs) %>%
  gather(key = Trait, value = value, BF_A:BF_O, na.rm = T) %>%
  ggplot(aes(x = Year, y = value, color = factor(LE_ChldBrth))) +
...
# no random slope
nested.mods <- nested.mods %>%
  mutate(mod1 = map(data, ~lmer(value ~ Year + le_value + Year:le_value + 
                                  (1|PROC_SID), data = .)),
         summary1 = map(mod1, ~print(summary(.))))
# add a random slope
nested.mods <- nested.mods %>%
  mutate(mod2 = map(data, ~lmer(value ~ Year + le_value + Year:le_value + 
                                  (Year|PROC_SID), data = .)),
         summary2 = map(mod2, ~print(summary(.))))

(nested.mods %>%
  mutate(comp = map2(mod1, mod2, anova)))$comp
```
...
nested.mods %>% unnest(pred1) %>%
  ggplot(aes(x = Year, y = pred, color = le_value)) +
  geom_line() + 
