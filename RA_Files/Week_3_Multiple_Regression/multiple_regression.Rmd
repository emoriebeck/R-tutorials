...
## Data  
```{r}
wd <- "~/Dropbox (Brown)/Summer 2018/RA Files/Week 3 Multiple Regression"
load("/Users/emoriebeck/Box/networks/JohnsonIPIP300andIPIP120.rdata")
...
```{r}
(m_results <- m_results %>%
  mutate_at(vars(estimate, se, p), funs(sprintf("%.2f", .))))
```
...
```{r}
(m_results <- m_results %>%
   gather(key = tmp, value = value, estimate, se, p))
```
