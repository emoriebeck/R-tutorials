...
dat.2005 <- dat.2005 %>%
  ...
  group_by(trait, PROC_SID, PROC_household, Dem_DOB, Dem_Sex, Psych_OthWorr) %>%
...
model_fun <- function(data){
  lm(value ~ age + Psych_OthWorr, data = data)
}
...
(pred.dat <- 
  crossing(
    age = seq(10, 80, 1),
    Psych_OthWorr = mean(dat.2005$Psych_OthWorr, na.rm = T)
  )) 
...
(nested.mods <- nested.mods %>%
  mutate(cat.int.model = map(data, model_fun),
         summary = map(cat.int.model, summary)))
...
(nested.mods <- nested.mods %>%
  mutate(int.pred = map(cat.int.model, pred_fun)))
...
```{r}
dat.2005 %>% select(age, Psych_OthWorr) %>% mutate(`age:PsychOthWorr` = age * Psych_OthWorr)
```

However, we have a problem. Neither of our variables have a natural 0 point. Since we're testing whether worries influence the relationship between age and personality, that's our moderator. In other words, worries is the variable that needs a natural 0 point. We only have one observation from each person, so person mean centering doesn't make sense. We also don't have a variable that it makes sense to center the value at, so we're going to grand mean center. 

```{r}
dat.2005 <- dat.2005 %>% 
  mutate(GMC_Psych_OthWorr  = Psych_OthWorr - mean(Psych_OthWorr, na.rm = T))
```
...
# write a new function for the interaction model
model_fun <- function(data){
  lm(value ~ age + GMC_Psych_OthWorr + age:GMC_Psych_OthWorr, data = data)
}

(nested.mods <- nested.mods %>%
  mutate(con.int.model = map(data, model_fun),
         summary = map(con.int.model, summary)))
```

Does worry moderate the relationship between age and personality?  

How would you interpret the age (main effect) coefficient?  

How would you interpret the worry (main effect) coefficient (hint: refer back to grand mean cetnering)?  
...
```{r}
m.psy.ow  <- mean(dat.2005$GMC_Psych_OthWorr, na.rm = T)
sd.psy.ow <-   sd(dat.2005$GMC_Psych_OthWorr, na.rm = T)

pred_fun <- function(model){
  crossing(
    age = seq(10,80,1),
    GMC_Pscyh_OthWorr = c(m.psy.ow - sd.psy.ow, m.psy.ow, m.psy.ow + sd.psy.ow)
  ) %>%
    mutate(pred = predict(model, newdata = .))
}
...
# let's get the predictions
(nested.mods <- nested.mods %>%
  mutate(int.pred = map(con.int.model, pred_fun)))
